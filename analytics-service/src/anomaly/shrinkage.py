...
from src.charts import apply_style, fig_to_base64, get_color_palette, add_watermark, format_currency, currency_formatter
...
def run(data: list[dict[str, Any]], params: dict[str, Any], chart_config: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if not expected_col or not actual_col:
        return {"success": False, "error": f"Need expected_stock and actual_stock columns. Found: {list(df.columns)}"}
    ...
    axes[0].axvline(mean_disc + threshold_sigma * std_disc, color="red", linestyle="--", label=f"+{threshold_sigma}s")
    axes[0].axvline(mean_disc - threshold_sigma * std_disc, color="red", linestyle="--", label=f"-{threshold_sigma}s")
    ...
    for _, row in sorted_df.head(100).iterrows():
        r = [str(row[product_col]), int(row[expected_col]), int(row[actual_col]),
             int(row["discrepancy"]), f"{row['discrepancy_pct']:.1f}%"]
        if cost_col:
            r.append(format_currency(row["shrinkage_value"], chart_config))
        r.extend([round(float(row["z_score"]), 2), "Yes" if row["flagged"] else "No"])
        table["rows"].append(r)
