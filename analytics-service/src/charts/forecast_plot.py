...
def run(data: list[dict[str, Any]], params: dict[str, Any], chart_config: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if lower_col and upper_col:
        ...
        ax.fill_between(df.loc[ci_mask, date_col], df.loc[ci_mask, lower_col], df.loc[ci_mask, upper_col],
    ...
    summary = {"totalDataPoints": len(df), **metrics}
    ...
    for _, row in df.tail(60).iterrows():
        ...
        if actual_col and pred_col:
            if pd.notna(row[actual_col]) and pd.notna(row[pred_col]):
                r.append(format_currency(row[actual_col] - row[pred_col], chart_config))
            else:
                r.append("---")
        table["rows"].append(r)
