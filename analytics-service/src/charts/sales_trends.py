...
def run(data: list[dict[str, Any]], params: dict[str, Any], chart_config: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if not data:
        return {"success": False, "error": "No data provided for sales trends chart"}
    ...
    for col in ["total_amount", "amount", "revenue", "total", "net_amount"]:
        if col in df.columns:
            ...
            break
    if not amount_col:
        return {"success": False, "error": "No amount column found. Expected: total_amount, amount, revenue, or total"}
