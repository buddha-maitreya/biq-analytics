...
def run(data: list[dict[str, Any]], params: dict[str, Any], chart_config: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if margin_col:
        ...
        return {"success": False, "error": "Need either margin column, or revenue + cost columns to compute margin"}

    x_col = qty_col or revenue_col
    if not x_col:
        return {"success": False, "error": "Need quantity or revenue column for X axis"}
