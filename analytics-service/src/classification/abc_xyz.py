...
def run(data: list[dict[str, Any]], params: dict[str, Any], chart_config: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if not product_col or not date_col:
        return {"success": False, "error": f"Missing required columns. Found: {list(df.columns)}"}
    ...
    if not revenue_col:
        return {"success": False, "error": "Need either amount or quantity column for ABC analysis"}
    ...
    for product in product_revenue.index:
        ...
        results.append({
            ...
            "abc": abc, "cv": round(float(cv), 3), "xyz": xyz, "segment": f"{abc}{xyz}",
        })
    ...
    for abc_class in ["A", "B", "C"]:
        ...
        axes[0].scatter(subset["cv"], subset["revenue"],
    ...
    summary = {
        ...
        "matrix": matrix,
        "thresholds": {"aThresholdPct": a_pct, "bThresholdPct": b_pct, "xCvThreshold": x_cv, "yCvThreshold": y_cv},
    }
