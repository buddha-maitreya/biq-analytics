...
def run(data: list[dict[str, Any]], params: dict[str, Any], chart_config: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if not customer_col:
        return {"success": False, "error": "No customer column found"}
    if not date_col:
    ...
    if len(df) == 0:
        return {"success": False, "error": "No data within analysis period"}
    ...
    segment_labels = params.get("segmentLabels", {
        "champions": [5, 4, 4], "loyal": [3, 3, 3], "potential_loyalists": [4, 2, 2],
        "at_risk": [2, 3, 3], "hibernating": [1, 1, 1], "lost": [1, 1, 2],
    })

    def assign_segment(row):
        r, f, m = row["R"], row["F"], row["M"]
        best_match, best_score = "other", -1
    ...
    seg_summary = rfm.groupby("segment").agg(
        count=(customer_col, "count"), avg_recency=("recency", "mean"),
    ...
    summary = {
        ...
        "segments": {
            row["segment"]: {
                "count": int(row["count"]), "avgRecencyDays": round(float(row["avg_recency"]), 1),
